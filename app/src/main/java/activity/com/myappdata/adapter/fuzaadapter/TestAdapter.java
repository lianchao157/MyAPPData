...
public class TestAdapter extends SectionedRecyclerViewAdapter<RecyclerView.ViewHolder,
    ...
    protected void onBindItemViewHolder(TestSectionBodyHolder holder, int section, int position) {
        int screenWidth = DisplayUtil.getScreenWidthPx((Activity) mContext);
        int imgWidth = (screenWidth - DisplayUtil.dip2px(mContext, 55 + 30)) / 3;
        ...
        if (position % 3 == 0) {
            params.leftMargin = DisplayUtil.dip2px(mContext, 55);
        } else if (position % 3 == 1) {
            params.leftMargin = DisplayUtil.dip2px(mContext, 35);
        } else {
            params.leftMargin = DisplayUtil.dip2px(mContext, 14);
        }
        params.bottomMargin = DisplayUtil.dip2px(mContext, 8);
