//package activity.com.myappdata.mvp.base.uimvp.adpter_mvp;
//
//import android.content.Context;
//import android.support.annotation.Nullable;
//
//import com.chad.library.adapter.base.BaseQuickAdapter;
////import com.chad.library.adapter.base.BaseViewHolder;
//import com.chad.library.adapter.base.viewholder.BaseViewHolder;
//
//import java.util.List;
//
//import activity.com.myappdata.R;
//import activity.com.myappdata.mvp.base.modelmvp.GoodInfiLineEntity.GoodInfoLineInfoBase;
//import activity.com.myappdata.mvp.base.modelmvp.entity.Goodemail;
//import activity.com.myappdata.mvp.base.modelmvp.entity.LogisticsINfoBean;
//
///***
// * 快递到哪了的线路代码210317
// */
//public class LogisticsInfoAdapter extends BaseQuickAdapter<GoodInfoLineInfoBase, BaseViewHolder> {
//    private Context context;
//    private List<GoodInfoLineInfoBase> data;
//
//    public LogisticsInfoAdapter(Context context, int layoutResId, @Nullable List<GoodInfoLineInfoBase> data) {
//        super(layoutResId, data);
//        this.context = context;
//        this.data = data;
//    }
//
////    @Override
////    protected void convert(BaseViewHolder helper, Goodemail item) {
////        //获取物流信息和物流时间的字体颜色, 最新的一条物流数据字体为绿色
////        int newInfoColor = context.getResources().getColor(helper.getLayoutPosition() == 0 ? R.color.green_btn_color_normal : R.color.gray);
////        //当前item的索引==0 && 物流数据的数量大于1条   ->  显示绿色大圆圈
////        helper.setGone(R.id.iv_new, helper.getLayoutPosition() == 0 && data.size() > 1)
////                //当前item的索引!=0 && 物流数据的数量大于1条   ->  显示灰色小圆圈
////                .setGone(R.id.iv_old, helper.getLayoutPosition() != 0 && data.size() > 1)
////                //当前item的索引 != 0    ->  显示圆点上面短一点的灰线
////                .setVisible(R.id.v_short_line, helper.getLayoutPosition() != 0)
////                //当前item的索引 != 物流数据的最后一条    ->  显示圆点下面长一点的灰线
////                .setGone(R.id.v_long_line, helper.getLayoutPosition() != data.size() - 1)
////                //当前item的索引 != 物流数据的最后一条    ->  显示物流时间下面的横向的灰线
////                .setGone(R.id.v_bottom_line, helper.getLayoutPosition() != data.size() - 1)
////                .setTextColor(R.id.tv_infoline, newInfoColor)
////                .setTextColor(R.id.tv_dateline, newInfoColor)
////                //物流信息
////                .setText(R.id.tv_infoline, item.getData()+"")
////                //物流时间
////                .setText(R.id.tv_dateline, item.getName()+"");
////    }
//
//    @Override
//    protected void convert(BaseViewHolder helper, GoodInfoLineInfoBase item) {
//        //获取物流信息和物流时间的字体颜色, 最新的一条物流数据字体为绿色
//        int newInfoColor = context.getResources().getColor(helper.getLayoutPosition() == 0 ? R.color.green_btn_color_normal : R.color.gray);
//        //当前item的索引==0 && 物流数据的数量大于1条   ->  显示绿色大圆圈
//        helper.setGone(R.id.iv_new, helper.getLayoutPosition() == 0 && data.size() > 1)
//                //当前item的索引!=0 && 物流数据的数量大于1条   ->  显示灰色小圆圈
//                .setGone(R.id.iv_old, helper.getLayoutPosition() != 0 && data.size() > 1)
//                //当前item的索引 != 0    ->  显示圆点上面短一点的灰线
//                .setVisible(R.id.v_short_line, helper.getLayoutPosition() != 0)
//                //当前item的索引 != 物流数据的最后一条    ->  显示圆点下面长一点的灰线
//                .setGone(R.id.v_long_line, helper.getLayoutPosition() != data.size() - 1)
//                //当前item的索引 != 物流数据的最后一条    ->  显示物流时间下面的横向的灰线
//                .setGone(R.id.v_bottom_line, helper.getLayoutPosition() != data.size() - 1)
//                .setTextColor(R.id.tv_infoline, newInfoColor)
//                .setTextColor(R.id.tv_dateline, newInfoColor)
//                //物流信息
//                .setText(R.id.tv_infoline, item.getPpshopGoodInfonoLocalcity() + "")
//                //物流时间
//                .setText(R.id.tv_dateline, item.getPpshopGoodInfonoData() + "");
//    }
//
////    @Override
////    protected void convert(BaseViewHolder helper, Goodemail item) {
////
////    }
//}
